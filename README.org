...
- This uses ids allover, the original has some parts left, where rowid is used
- init git repository if the url is already known
- fix in sync.
- use remote database as master for merging to keep number of entries lower and allow cleanup
